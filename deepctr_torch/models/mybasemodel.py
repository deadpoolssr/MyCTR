...
class Linear(nn.Module):
    ...
    def forward(self, X):
        ...
        #     print(range(self.feature_index[feat.name][0],self.feature_index[feat.name][1]))
            # print(torch.sum(tmp))
...
class BaseModel(nn.Module):
    def __init__(self, linear_feature_columns, dnn_feature_columns, l2_reg_linear=1e-5, l2_reg_embedding=1e-5,
        ...
        #     print(feat.name+':'+str(feat.vocabulary_size))
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            if do_validation:
                eval_result = self.evaluate(val_x, val_y, batch_size, epoch)
    ...
    def fit_one_epoch(self, x, y, batch_size=None, shuffle=True, verbose=0, epoch= 0, validation_data=None):
        ...
        steps_per_epoch = (sample_num - 1) // batch_size + 1
        # Train
        print("Train on {0} samples {1} steps per epoch".format(len(train_tensor_data), steps_per_epoch))
        epoch_logs = {}
        start_time = time.time()
        loss_epoch = 0
        total_loss_epoch = 0
    ...
    def evaluate(self, x, y, batch_size, epoch):
        """
        ...
        """
        pred_ans = self.predict(x, batch_size, epoch)
        eval_result = {}
        for name, metric_fun in self.metrics.items():
            eval_result[name] = metric_fun(y, pred_ans)
        return eval_result

    def predict(self, x, batch_size, epoch):
        """
    ...
    def input_from_feature_columns(self, X, feature_columns, embedding_dict, support_dense=True):
        ...
        #     print(torch.max(X[:, self.feature_index[feat.name][0]:self.feature_index[feat.name][1]].long()))
        #     print(torch.min(X[:, self.feature_index[feat.name][0]:self.feature_index[feat.name][1]].long()))
        #     print(embedding_dict[feat.embedding_name].num_embeddings)
            # print(embedding_dict[feat.embedding_name](X[:, self.feature_index[feat.name][0]:self.feature_index[feat.name][1]].long()).size())

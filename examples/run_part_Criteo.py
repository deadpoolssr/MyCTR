...
if __name__ == "__main__":
    ...
    test = read_part_data('part1', sparse_features)
    valid = read_part_data('part2', sparse_features)


    # dense_features = []
    for epoch in range(epoch_num):
        for i in range(3,11):
            train = read_part_data('part' + str(i), sparse_features)
            model.fit(train_model_input, train[target].values, batch_size=1024, epochs=1, verbose=2, validation_data=0.1)
